Function Connect-MyAccountOldPortal{
    [CmdletBinding()]
    Param(
    ...
    Process{
        ...
        if($null -ne $data){
            $cookie = $data.Cookies | Where-Object {$_.Name -eq '.AspNet.Cookies'}
            if($null -ne $cookie -and $cookie -is [System.Net.Cookie] -and $null -ne $cookie.Value){
                #Write message
                $msg = @{
                    MessageData = ($Script:messages.SuccessfullyConnectedTo -f "My account old portal")
    ...
    End{
        if($null -ne (Get-Variable -Name results -ErrorAction Ignore)){
            ...
            $results.microsoft_myaccount_oldPortal = $obj
        }
    }
}