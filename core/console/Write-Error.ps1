    ...
    begin
    {
        try {
            ...
            elseif($PSBoundParameters.ContainsKey('ErrorAction')){
                $informationAction = $PSBoundParameters.ErrorAction
            }
            elseif($PSBoundParameters.ContainsKey('informationAction') -and [System.String]::IsNullOrEmpty($PSBoundParameters.informationAction)){
                $informationAction = "SilentlyContinue"
            }
            else{
                $informationAction = $ErrorActionPreference            
            }
            #Check Log Level
            if($PSBoundParameters.ContainsKey('logLevel') -and ($null -eq $PSBoundParameters.logLevel -or [String]::IsNullOrEmpty($PSBoundParameters.logLevel))){
                $PSBoundParameters.logLevel = 'error'
            }
            ...
            if($formattedMessage){
                $formattedMessage = ("ERROR: {0}" -f $formattedMessage)
            }
            #Set color
